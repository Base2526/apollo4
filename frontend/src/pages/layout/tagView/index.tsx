...
import { addTag, removeTag, setActiveTag } from '@/action/tags-view.store';

import type { TagState } from '@/interface/layout/tagsView.interface';
...
const TagsView: FC = () => {
  ...
  const { tags, activeTagId } = useSelector((state: {tagsView: TagState} )=> state.tagsView)
  const { menuList, locale } = useSelector((state: { user: UserState }) => state.user);
