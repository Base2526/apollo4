...
import { query_members, mutation_calcute_recheck } from "@/apollo/gqlQuery";
...
const CalcuteRecheckPage: React.FC = (props) => {
  ...
  const [onCalcuteRecheck] = useMutation(mutation_calcute_recheck, {
    context: { headers: getHeaders(location) },
    update: (cache, { data: { calcute_recheck } }) => {
      console.log("calcute_recheck :", calcute_recheck);

      let { status, data} = calcute_recheck;
      // if(status){
      //   // setData(data)
  ...
  const handleSubmit = () => {
    ...
    if (selectedRange) {
      ...
      setLoading(true)
      onCalcuteRecheck({ variables: { input: { startDate, endDate } } });
    }
  };
...
export default CalcuteRecheckPage;